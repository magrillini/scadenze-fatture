...
<head>
    ...
    <title><?= $currentPage === 'controllo' ? 'CONTROLLO' : 'Controllo Fatture XML' ?></title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .page-actions,.hero-badges,.checkbox-grid,.table-tools,.pie-wrapper { display: flex; gap: 12px; flex-wrap: wrap; align-items: center; }
        ...
        .home-empty, .image-editor, .pie-card { border-radius: 16px; }
        .home-empty { position: relative; z-index: 1; padding: 20px; border: 1px dashed rgba(255,255,255,.4); background: rgba(255,255,255,.1); }
        .image-editor, .pie-card { border: 1px solid #e5e7eb; padding: 16px; background: #f9fafb; }
...
<div class="container">
    ...
    <?php if ($currentPage === 'home'): ?>
        <section class="card hero"><div class="hero-content"><div><h1>CONTROLLO FATTURE XML</h1><p>La home mostra i grafici generali. Con il pulsante <strong>CONTROLLO</strong> apri l'area dedicata ai monitoraggi operativi, alle ricerche e alle modifiche sulle fatture.</p></div><div class="hero-badges"><span class="hero-badge">Scadenze totali: <?= (int) $summary['total_dues'] ?></span><span class="hero-badge">Da riscuotere: <?= euro((float) $summary['outstanding_amount']) ?></span><span class="hero-badge">Clienti monitorati: <?= count($summary['customers']) ?></span></div><p><a class="button secondary" href="/controllo.php">Apri CONTROLLO</a></p><?php if ($selectedHomeVariant['images'] !== []): ?><div class="home-gallery layout-<?= (int) $selectedHomeVariant['layout'] ?>"><?php foreach ($selectedHomeVariant['images'] as $image): ?><figure class="home-photo"><img src="<?= htmlspecialchars($image['path']) ?>?v=<?= urlencode((string) ($image['updated_at'] ?? '1')) ?>" alt="<?= htmlspecialchars($image['title']) ?>"><figcaption><strong><?= htmlspecialchars($image['title']) ?></strong><span><?= htmlspecialchars($image['caption']) ?></span></figcaption></figure><?php endforeach; ?></div><?php else: ?><div class="home-empty"><strong>Nessuna foto configurata.</strong><p>Apri l'area superadmin per caricare da 1 a 5 immagini e attivare i layout casuali della home.</p></div><?php endif; ?></div></section>
        ...
        <div class="card"><h2>Grafici generali</h2><div class="chart"><?php $overviewMax = max($summary['charts']['overview'] ?: [0]); foreach ($summary['charts']['overview'] as $label => $value): ?><div class="bar-row"><strong><?= htmlspecialchars($label) ?></strong><div class="bar-track"><div class="bar-fill" style="width: <?= number_format(percentOf((float) $value, (float) $overviewMax), 2, '.', '') ?>%;"></div></div><span class="amount"><?= euro((float) $value) ?></span></div><?php endforeach; ?></div></div>

        <div class="card"><h2>Grafici a torta scadenze</h2><p class="muted">Rosso per fatture scadute non pagate, arancione per fatture entro 19 giorni dalla scadenza, verde per fatture oltre 20 giorni dalla scadenza.</p><div class="pie-grid"><div class="pie-card"><h3>Distribuzione per importo totale IVA</h3><div class="pie-wrapper"><div class="pie-chart" style="background: <?= htmlspecialchars(buildPieGradient($pieByAmount)) ?>;"></div><div class="legend"><?php foreach ($pieSegments as $segment): ?><div class="legend-item"><span class="legend-color" style="background: <?= htmlspecialchars($segment['color']) ?>;"></span><span><?= htmlspecialchars($segment['label']) ?></span><strong><?= euro((float) $segment['value']) ?></strong></div><?php endforeach; ?></div></div></div><div class="pie-card"><h3>Distribuzione per numero fatture</h3><div class="pie-wrapper"><div class="pie-chart" style="background: <?= htmlspecialchars(buildPieGradient($pieByCount)) ?>;"></div><div class="legend"><?php foreach ($pieSegments as $segment): ?><div class="legend-item"><span class="legend-color" style="background: <?= htmlspecialchars($segment['color']) ?>;"></span><span><?= htmlspecialchars($segment['label']) ?></span><strong><?= (int) $segment['count'] ?></strong></div><?php endforeach; ?></div></div></div></div></div>
    <?php else: ?>
        <section class="card"><h1>CONTROLLO</h1><p class="muted">Qui trovi i monitoraggi operativi, i filtri, la gestione pagamenti e il dettaglio delle fatture.</p></section>
