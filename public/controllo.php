...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .bar-track { background: #e5e7eb; border-radius: 999px; overflow: hidden; min-height: 16px; display: flex; }
        .bar-fill-segment { min-height: 16px; }
        .segment-overdue { background: #dc2626; }
        .segment-future { background: #2563eb; }
        .segment-soon { background: #f97316; }
        .segment-paid { background: #16a34a; }
        .bar-labels { position: relative; min-height: 18px; font-size: 12px; color: #374151; }
        .bar-label { position: absolute; transform: translateX(-50%); white-space: nowrap; font-weight: 700; }
...
<div class="container">
    ...
    <section class="card"><h2>Grafico clienti / CF</h2><p class="muted">Ordinato per grandezza di cifra e raggruppato per <?= $chartGroupBy === 'cf' ? 'codice fiscale / partita IVA' : 'cliente' ?>.</p><div class="section-toolbar"><div class="search-input-wrap"><span class="search-icon" aria-hidden="true">🔍</span><input id="customer_chart_search" type="search" autocomplete="off" placeholder="Cerca nel grafico (minimo 3 caratteri)"></div><button type="button" class="button all-customers" id="customer_chart_show_all">VEDI TUTTI</button></div><p class="muted" id="customer_chart_hint">Grafico compattato: digita almeno 3 caratteri o premi VEDI TUTTI.</p><div class="chart"><?php foreach ($summary['customers'] as $customer): $label = $chartGroupBy === 'cf' ? ($customer['cf'] ?: '-') : $customer['client']; $searchKey = mb_strtolower(trim((string) ($customer['client'] ?? '')) . ' ' . trim((string) ($customer['cf'] ?? ''))); $totalTurnover = (float) ($customer['turnover'] ?? 0); $segments = [['class' => 'segment-future', 'amount' => (float) ($customer['future_amount'] ?? 0.0)], ['class' => 'segment-soon', 'amount' => (float) ($customer['due_soon_amount'] ?? 0.0)], ['class' => 'segment-paid', 'amount' => (float) ($customer['paid_amount'] ?? 0.0)], ['class' => 'segment-overdue', 'amount' => (float) ($customer['overdue_unpaid_amount'] ?? 0.0)]]; ?><div class="bar-row customer-chart-row" data-search="<?= htmlspecialchars($searchKey) ?>"><div><strong><?= htmlspecialchars($label) ?></strong><br><span class="muted"><?= htmlspecialchars($customer['client']) ?></span> — <span class="stars"><?= htmlspecialchars(renderStars((int) $customer['stars'])) ?></span></div><div class="bar-stack"><div class="bar-labels"><?php $offset = 0.0; foreach ($segments as $segment): $segmentAmount = (float) $segment['amount']; if ($segmentAmount <= 0 || $totalTurnover <= 0) { continue; } $segmentPercent = ($segmentAmount / $totalTurnover) * 100; $labelPosition = $offset + ($segmentPercent / 2); ?><span class="bar-label" style="left: <?= number_format($labelPosition, 2, '.', '') ?>%;"><?= euro($segmentAmount) ?></span><?php $offset += $segmentPercent; endforeach; ?></div><div class="bar-track" style="width: 100%;"><?php foreach ($segments as $segment): $segmentAmount = (float) $segment['amount']; if ($segmentAmount <= 0 || $totalTurnover <= 0) { continue; } ?><div class="bar-fill-segment <?= htmlspecialchars((string) $segment['class']) ?>" style="width: <?= number_format(($segmentAmount / $totalTurnover) * 100, 2, '.', '') ?>%;" title="<?= euro($segmentAmount) ?>"></div><?php endforeach; ?></div></div><span class="amount">Totale anno corrente: <?= euro((float) ($customer['turnover_current_year'] ?? 0.0)) ?></span></div><?php endforeach; ?></div></section>
